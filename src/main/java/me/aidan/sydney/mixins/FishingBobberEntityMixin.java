...
public class FishingBobberEntityMixin implements IMinecraft {
    ...
    private void pushOutOfBlocks(FishingBobberEntity instance, Entity entity, Operation<Void> original) {
        if (entity == mc.player && ISU.MODULE_MANAGER.getModule(VelocityModule.class).isToggled() && ISU.MODULE_MANAGER.getModule(VelocityModule.class).antiFishingRod.getValue())
            return;
